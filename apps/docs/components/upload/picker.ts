...
export async function openCropPicker(_options: BasicUploadOptions) {
  ...
  const currentCount = _options.currentCount ?? 0
  const maxCount = _options.maxCount ?? Infinity
  if (currentCount + files.length > maxCount) {
    files = files.slice(0, maxCount - currentCount)
  }
  ...
  for (const file of files) {
    if (isImage(file.mime)) {
      ...
      if (!shouldPrint || !Array.isArray(_options.watermark) || !_options.watermark.length) {
        continue
      }
...
export async function openDocumentPicker(options: BasicUploadOptions) {
  ...
  if (currentCount + resolvedFiles.length > maxCount) {
    resolvedFiles = files.slice(0, maxCount - currentCount)
  }
...
export const composedPicker: {
  ...
  documentPicker: openDocumentPicker as any,
}
