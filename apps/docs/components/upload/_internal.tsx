...
import type { CustomPreview, PreviewInstance } from './components/preview/Preview';
...
import { Toast } from '..';
import { ToastMethods } from '../toast/types';
import { RegularCount } from '../uploader/interface';
...
const _UploadInternal: ForwardRefRenderFunction<UploadInstance, UploadProps> = (
  ...
  useImperativeHandle(ref, () => ({
    open: chooseResourceAndUpload as any,
  }));
  // 受控情形下的内外数据同步
  useEffect(() => {
  ...
  function removeImage(item: UploadItem) {
    ...
    if (isDef(count)) {
      results[targetIndex] = null as any;
    } else {
  ...
  async function chooseResourceAndUpload(config: OverrideUploadConfig) {
    try {
      ...
      for (const file of filesToUpload) {
        ...
        const idx = valueCopy.current.findIndex((_file) => _file?.key === uploadRes.key);
        if (~idx) {
  ...
  async function handleReupload(item: ISource) {
    ...
    const res = await getFileBeforeUpload({
      uri: valueCopy.current[currIndex].uri as any,
      name: '',
      type: valueCopy.current[currIndex].type as any,
    });
    ...
    <>
      {typeof count === 'undefined' ? (
        ...
        <Uploader.Regular
          onPressImage={handlePress}
          count={count}
          onPressDelete={handleDelete as any}
          onPressUpload={handlePressAdd}
          onPressError={handleReupload}
          list={value}
          colCount={imagesPerRow}
